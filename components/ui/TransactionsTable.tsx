        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <SortButton field="physicalBookId" label="Physical Book ID" />
              </TableHead>
              <TableHead>
                <SortButton field="userId" label="User ID" />
              </TableHead>
              <TableHead>
                <SortButton field="adminId" label="Admin ID" />
              </TableHead>
              <TableHead>
          ...
          <TableBody>
            {sortedTransactions?.map((tx) => (
              <TableRow key={tx.tid}>
                ...
                <td className="px-4 py-2">{tx.adminId}</td>
                ...
                <td className="px-4 py-2 flex gap-2">
                  ...
                  {tx.status !== "accepted" && tx.status !== "rejected" && (
                    <Button size="sm" onClick={() => handleAccept(tx.tid)} className="bg-green-500 hover:bg-green-600 text-white" disabled={loadingTransaction === tx.tid}>
                      {loadingTransaction === tx.tid ? "Loading..." : "Accept"}
                    </Button>
                  )}
                  ...
                  {tx.status !== "accepted" && tx.status !== "rejected" && (
                    <Button size="sm" onClick={() => handleReject(tx.tid)} className="bg-red-500 hover:bg-red-600 text-white" disabled={loadingTransaction === tx.tid}>
                      {loadingTransaction === tx.tid ? "Loading..." : "Reject"}
                    </Button>
                  )}
                </td>
              </TableRow>
